...
That's rather disappointing. Almost all the data fall into two bins. The bins go to up to about 50 thousand, indicating that some values are very high, but their content is small and we can't really see their distribution. Producing the same plot using a logarithmic scale for the _x_-axis may tell us more.
```{r}
histogram(~integral, hits, nint=50, type="count",
...
What we want to study is the relation between the `sumadc` for each cluster and the sum of `integral` for all the hits in that cluster.
```{r}
xyplot(sumadc~sumint,
       hc[, .(sumadc=first(sumadc), sumint=sum(integral)), .(eid.1,eid.2,eid.3,id)],
...
Another possibility is to visualize a 2-d kernel density estimate:
```{r}
...
```{r}
hc$gofbin <- cut(hc$gof, breaks=c(-1, 0, 10, 100, 1000, 10000), include.lowest=TRUE)
summary(hc$gofbin)
```
