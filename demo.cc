...
int main(int argc, char** argv) try
{
    ...
    TH1F nparticles_hist("nparticles", "Number of particles per MCTruth", 51,
                         -0.5, 50.5);
    ...
    TH2F xyhist("vertex_xy", "x vs. y for each vertex", 20, -400., 400., 20, -600., 600.);

    TH2F nclus_vs_adc_sum("nclus_vs_adc_sum", "number of clusters vs. ADC sum",
                          30, 0., 30.,
                          20, 4000., 140000.);
    ...
    for (gallery::Event ev(filenames); !ev.atEnd(); ev.next()) {
        ...
        analyze_vertex_cluster_correlations(ev, vertices_tag, vertex_cluster_assns,
                                            nclus_vs_adc_sum);
        analyze_cluster_hit_correlations(ev, clusters_tag, cluster_hit_assns,
                                         adc_vs_summed_integrals);
        times.push_back(duration_cast<microseconds>(system_clock::now() - t0));
    }

    auto const elapsed_time = duration_cast<milliseconds>(system_clock::now() - start_time);
    auto const sum_times = std::accumulate(begin(times), end(times), microseconds(0));
